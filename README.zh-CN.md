...
- **Skill 与 MCP 可直接复制。** 首页可以直接复制项目 `SKILL.md` 和 MCP 配置模板，方便分享给别人使用。
...
首页把两个概念分开，不再混成一组功能卡片：

- **Skill** 是当前项目实现方式的操作手册，首页直接提供可复制的 `SKILL.md`，源文件在 [`skills/ai-ppt-edit/SKILL.md`](skills/ai-ppt-edit/SKILL.md)。
- **MCP 服务** 是后续可被外部客户端调用的工具层，包括文件系统工作区、浏览器渲染/导出、模板注册表；首页提供可复制的 MCP 配置模板。
...
开发环境下，Vite 会把 `/v1/*` 代理到 Fastify API：`http://localhost:3000`。这个接口只返回安全的产品 manifest，不读取你的个人 MCP 配置，也不会暴露本地路径或密钥。
